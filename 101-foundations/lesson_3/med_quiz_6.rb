...
# d_outer is 42 with an id of: 85 before the block.
  ...
  puts "a_outer is #{a_outer} with an id of: #{a_outer_id} BEFORE and: #{a_outer.object_id} AFTER the block." ###### UNEXPECTED
  puts "b_outer is #{b_outer} with an id of: #{b_outer_id} BEFORE and: #{b_outer.object_id} AFTER the block."
  puts "c_outer is #{c_outer} with an id of: #{c_outer_id} BEFORE and: #{c_outer.object_id} AFTER the block."
  puts "d_outer is #{d_outer} with an id of: #{d_outer_id} BEFORE and: #{d_outer.object_id} AFTER the block.\n\n"  

# a_outer is 22 with an id of: 85 BEFORE and: 45 AFTER the block.  ###### UNEXPECTED
...
# (notice that each variable keeps its new object/object_id even when we leave the block.)   ###### UNEXPECTED
...
# d_outer is 42 with an id of: 85 before the block.
  ...
  puts "a_outer is #{a_outer} with an id of: #{a_outer_id} BEFORE and: #{a_outer.object_id} AFTER the method call."   ###UNEXPECTED
  puts "b_outer is #{b_outer} with an id of: #{b_outer_id} BEFORE and: #{b_outer.object_id} AFTER the method call."
  puts "c_outer is #{c_outer} with an id of: #{c_outer_id} BEFORE and: #{c_outer.object_id} AFTER the method call."
  puts "d_outer is #{d_outer} with an id of: #{d_outer_id} BEFORE and: #{d_outer.object_id} AFTER the method call.\n\n"

# a_outer is 42 with an id of: 85 BEFORE and: 85 AFTER the method call.                             ###UNEXPECTED
...
def generate_uuid(sections=5)
  uuid = []
  charset = %w{0 1 2 3 4 6 7 9 A B C D E F G H J K M N O P Q R T V W X Y Z}
  sections.times do

...
class WorldTest < Minitest::Test
  ...
  def test_after_adding_a_cell_the_world_is_not_empty
    world = World.new
    world.create_cell(Location.new(1, 2, world.board))
    refute world.empty?
  end

  def test_can_add_location
    world = World.new
    location = Location.new(1, 2, world.board)
  ...
  def test_remains_alive_in_next_generation_with_two_neighbors
    ...
    world.create_cell(Location.new(1, 1, world.board))
    world.create_cell(Location.new(2, 2, world.board))
    assert cell.alive_in_next_generation?
  end
